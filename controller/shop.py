    ...
    def update_shop(shop_id: int, update_data: ShopUpdate):
        try:
            with DBSession() as db:
                ...
                shop = db.query(Shop).filter(Shop.id == shop_id).first()
                if not shop:
                    raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Shop not found")
            ...
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Error updating shop: {str(e)}")
